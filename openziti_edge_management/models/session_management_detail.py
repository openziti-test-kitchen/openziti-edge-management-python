...
class SessionManagementDetail(BaseModel):
    """
    ...
    type: DialBind = Field(...)
    service_policies: Optional[conlist(EntityRef)] = Field(None, alias="servicePolicies")
    __properties = ["_links", "createdAt", "id", "tags", "updatedAt", "apiSession", "apiSessionId", "edgeRouters", "identityId", "service", "serviceId", "token", "type", "servicePolicies"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SessionManagementDetail:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in service_policies (list)
        _items = []
        if self.service_policies:
            for _item in self.service_policies:
                if _item:
                    _items.append(_item.to_dict())
            _dict['servicePolicies'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> SessionManagementDetail:
        """Create an instance of SessionManagementDetail from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SessionManagementDetail.parse_obj(obj)

        _obj = SessionManagementDetail.parse_obj({
            "links": dict(
            ...
            "token": obj.get("token"),
            "type": obj.get("type"),
            "service_policies": [EntityRef.from_dict(_item) for _item in obj.get("servicePolicies")] if obj.get("servicePolicies") is not None else None
        })
        return _obj
